use dqcsim::{
    common::{
        ...
        gates::*,
        types::{
            ArbData, Basis, Gate, Matrix, PluginMetadata, PluginType, QubitMeasurementValue,
            QubitRef,
        },
    },
...
impl<'state, 'def> Program<'state, 'def> {
    ...
    /// Sends the barrier instruction as a custom gate when the environment
    /// variable `DQCSIM_OPENQASM_BARRIER` is set.
    fn barrier(&mut self, args: &Argument) -> Result<()> {
        match env::var_os("DQCSIM_OPENQASM_BARRIER") {
            Some(_) => self.state.gate(Gate::new_custom(
                "openqasm.barrier",
            ...
            None => {
                warn!("DQCSIM_OPENQASM_BARRIER is not set. Skipping barrier operation.");
                Ok(())
            }
        }
    }

    /// Resets given qubits to the |0> state.
    fn reset(&mut self, arg: &Argument) -> Result<()> {
        self.state.gate(Gate::new_prep(
            self.get_qubits(arg)?,
            Matrix::from(Basis::Z),
        )?)
    }
    ...
    fn measure(&mut self, qreg: &Argument, creg: &Argument) -> Result<()> {
        ...
        self.state.gate(Gate::new_measurement(
            qubits.clone(),
            Matrix::from(Basis::Z),
        )?)?;
            ...
            "U" => {
                ...
                gates.append(
                    self.get_qubits(&qubits[0])?
                        .into_iter()
                        .map(|q| {
                            let a = (params[0] / 2.).cos();
                            let b = (params[0] / 2.).sin();
                            let c = (params[1] + params[2]) / 2.;
                            let d = (params[1] - params[2]) / 2.;
                            Gate::new_unitary(
                                vec![q],
                                vec![],
                                vec![
                                    Complex64::new(0., -c).exp() * a,
                                    -Complex64::new(0., -d).exp() * b,
                                    Complex64::new(0., d).exp() * b,
                                    Complex64::new(0., c).exp() * a,
                                ],
                            )
